...
public class TypeBuilder : ITypeBuilder {
    ...
    private void GenerateClass(string interfaceName) {
        ...
        if(_isOperation) {
            dataMayBeNull = true;
            _stringBuilder.AppendLine($"\tpublic {_typeName}(JsonNode? errors, JsonNode? data) {{");
            _stringBuilder.AppendLine("\t\tErrors = errors?.Deserialize<IEnumerable<ClientError>>()?.ToArray() ?? new IClientError[0];");
        } else {
