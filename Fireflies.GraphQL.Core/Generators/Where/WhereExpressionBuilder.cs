...
internal class WhereExpressionBuilder<TElement> : ASTVisitor<IGraphQLContext> {
    ...
    protected override async ValueTask VisitObjectFieldAsync(GraphQLObjectField objectField, IGraphQLContext context) {
        ...
        if(objectField.Name.StringValue.Equals(nameof(Where<int>.Equal), StringComparison.InvariantCultureIgnoreCase)) {
            await CreateOperationExpression(objectField, false, Expression.Equal);
            return;
        }

        if(objectField.Name.StringValue.Equals(nameof(Where<int>.NotEqual), StringComparison.InvariantCultureIgnoreCase)) {
            await CreateOperationExpression(objectField, false, Expression.NotEqual);
            return;
        }
    ...
    private async Task CreateOperationExpression(GraphQLObjectField objectField, bool negate, Func<Expression, ConstantExpression, Expression> factory) {
        ...
        var value = await _valueAccessor.GetValue(objectField);
